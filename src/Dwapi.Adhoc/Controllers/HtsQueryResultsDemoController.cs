...
{
    //[Authorize]
    public class HtsQueryResultsDemoController : Controller
    {
        private string _instanceId = "HtsQueryResults";
        ...
        public HtsQueryResultsDemoController(IQueryBuilderService aqbs, IQueryTransformerService qts,IConfiguration configuration)
        {
            _aqbs = aqbs;
            _qts = qts;
        ...
        private ActionResult GetData(QueryTransformer qt, Param[] _params)
        {
            ...
            var conn = new MySqlConnection(_configuration.GetConnectionString("HtsSourceConnection"));
            var sql = qt.SQL;

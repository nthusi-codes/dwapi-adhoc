...
{
    public class QueryTransformerMsSqlStoreProvider : IQueryTransformerProvider
    {
        ...
        public QueryTransformerMsSqlStoreProvider(IQueryBuilderService aqbs, IHostingEnvironment env, IConfiguration config)
        {
            _aqbs = aqbs;
            _env = env;
            _config = config;
            ...
            SaveState = true;
            _connection = DataBaseHelper.CreateMsSqlConnection(GetDatabasePath());

            var sql =
                @"
                if not exists (select * from sysobjects where name='QueryTransformers' and xtype='U')
                create table QueryTransformers
                        (
	                        id nvarchar(50)
		                        constraint QueryTransformers_pk
        ...
        {
            var con = Path.Combine(_config.GetConnectionString("SourceConnection"));
            return con;
        }
